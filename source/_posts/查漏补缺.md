...
### 10. 原型上的方法和对象上的方法有哪些区别

1. 对象方法包括构造函数中的方法以及构造函数原型上面的方法，需要通过实例化对象去调用；

2. 类方法，其实这里的类就是一个函数，在js中由于函数也是一个对象，所以可以为函数添加属性以及方法，这种方法在node中用的比较多，不需要通过实例化对象去调用；

3. 原型方法一般用于对象实例共享，比如`Person.prototype.sayName=function(){console.log(this.name);};`在原型上面添加该方法，就能实现共享。这样就不用每一次初始化一个实例的时候，为其分配相应的内存了，原型方法也需要通过实例化对象去调用。
...
### 1. webpack打包后为同一个文件，其中的变量相同的为什么不会冲突

## Http篇

### 1. 一个url从浏览器输入到页面显示发生了什么

查找IP – 建立握手 – 请求 – 服务器处理 – 关闭连接 – 资源解析 – 网页渲染

### 2. Http的缓存有哪些

200：

返回头中的Expires或者Cache-Control
...
类似node中间件代理，需要搭建一个中转nginx服务器，用于转发请求。最简单的跨域方式。只要修改nginx的配置即可解决跨域问题。

#### cors

cors需要浏览器和后台同时支持。 ie8和ie9需要通过XDomainRequest实现。浏览器会自动进行cors通信，所以只要后台实现了cros，就实现了跨域。
...
### 如何实现一个弹框组件（多级嵌套后如何保证 fixed 不被其他定位影响）
